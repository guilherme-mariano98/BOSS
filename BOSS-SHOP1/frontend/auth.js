...
async function handleRegister(formData) {
    try {
        ...
        if (response.ok) {
            ...
            // Handle specific error messages from Node.js API
...
document.addEventListener('DOMContentLoaded', function() {
        ...
        resetForm.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    closeResetModal();
                    // Para demonstração local, exibir token no console
                    console.log('Password reset token:', data.token);
                } else {
                    showToast(data.error || 'Erro ao solicitar recuperação', 'error');
                }
            } catch (err) {
    ...
    if (loginForm) {
        loginForm.addEventListener('submit', async function(e) {
            ...
            if (!validateForm('loginFormSubmit')) return;
    ...
    if (registerForm) {
        registerForm.addEventListener('submit', async function(e) {
            ...
            if (!validateForm('registerFormSubmit')) return;
