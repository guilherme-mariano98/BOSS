        ...
        const demoUsers = [
            {
                ...
                cpf: '123.456.789-00',
                gender: 'male',
                address: 'Rua Administrador, 100, Centro',
                city: 'São Paulo',
                state: 'SP',
                zipCode: '01000-000',
                country: 'Brasil',
            ...
            },
            {
                ...
                cpf: '987.654.321-00',
                gender: 'female',
                address: 'Av. Cliente, 456, Jardim das Flores',
                city: 'São Paulo',
                state: 'SP',
                zipCode: '04567-890',
                country: 'Brasil',
...
async function handleLogin(formData) {
    try {
        ...
        const email = formData.get('email') || document.getElementById('loginEmail')?.value || document.getElementById('email')?.value;
        const password = formData.get('password') || document.getElementById('loginPassword')?.value || document.getElementById('password')?.value;
...
async function handleRegister(formData) {
    try {
        ...
        const email = formData.get('email') || document.getElementById('email')?.value || document.getElementById('registerEmail')?.value;
        const password = formData.get('password') || document.getElementById('password')?.value || document.getElementById('registerPassword')?.value;
        const phone = formData.get('phone') || document.getElementById('phone')?.value || '';
        const birthDate = formData.get('birthDate') || document.getElementById('birthDate')?.value || '';
        const cpf = formData.get('cpf') || document.getElementById('cpf')?.value || '';
        const gender = formData.get('gender') || document.getElementById('gender')?.value || '';
        ...
        const state = formData.get('state') || document.getElementById('state')?.value || '';
        const zipCode = formData.get('zipCode') || document.getElementById('zipCode')?.value || '';
        const newsletter = formData.get('newsletter') === 'true' || document.getElementById('newsletter')?.checked || false;
        const smsNotifications = formData.get('smsNotifications') === 'true' || document.getElementById('smsNotifications')?.checked || false;
        
        if (!firstName || !lastName || !email || !password) {
            return { success: false, message: 'Nome, sobrenome, email e senha são obrigatórios' };
        }
        ...
        const newUser = {
            ...
            phone: phone,
            birthDate: birthDate,
            dateOfBirth: birthDate, // Compatibilidade
            cpf: cpf,
            gender: gender,
            address: address,
            city: city,
        ...
        return { 
            success: true, 
            message: 'Conta criada com sucesso! Bem-vindo à BOSS SHOPP!',
            user: userData,
            token: token
        };
